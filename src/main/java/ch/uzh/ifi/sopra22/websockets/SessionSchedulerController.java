...
public class SessionSchedulerController {
     ...
     */
    @MessageMapping("/sessionScheduler")
    @SendTo("/topic/sessionScheduler")
    public TaskMessage send(final TaskMessage message){
        System.out.println(message.toString());
        return new TaskMessage(message.getTaskID(), message.getUser(), message.getAction());
    }
}
