...
public class UserController {
  ...
  public UserGetLoginDTO createUser(@RequestBody UserPostDTO userPostDTO) {
    ...
    //login user, maybe POST is wrong method
    ...
    public UserGetLoginDTO loginUser(@RequestBody UserPostDTO userPostDTO) {
        ...
        // convert internal representation of user back to API
        return DTOMapper.INSTANCE.convertEntityToUserGetLoginDTO(loggedUser);
    }
    ...
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public UserGetDTO returnOneUser(@PathVariable("id") Long id) {

        User returnUser = userService.returnUserByID(id);
    ...
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public UserGetDTO updateUser(@PathVariable("id") Long id, @RequestBody UserPutDTO userIn) {

        // convert API user to internal representation
        //now there is a nice array, that I can easily access
        User userInput = DTOMapper.INSTANCE.convertUserPutDTOtoEntity(userIn);

        User returnUser = userService.updateUser(id, userInput);
        //will update the data and look at token match
